...
Last Updated: 2026-07-09 15:31 JST
...
## 2026-07-09 — Reading Accessibility

### Added

- 「生成AIのしくみ」平易版・詳解版の各章ページに、記事本文用の文字サイズ切替を追加した。
- 文字サイズは、小 / 標準 / 大 / 特大から選べるようにし、選択状態を `localStorage` に保存してページ間で引き継ぐようにした。

### Changed

- スマートフォンでの本文既定サイズを読みやすい水準に保ち、段落、見出し、リスト、中心フレーズなどが同じ読書スケールに従うようにした。
- Appearance Theme の Paper / Reading / Archive と共存するよう、文字サイズ切替ボタンの色と選択状態をテーマ変数に連動させた。

### Notes

この変更は、読者からの「スマートフォンで本文が小さい」という報告を受けた読書アクセシビリティ改善である。
