...
Last Updated: 2026-07-22 22:50 JST
...
## Publishing Structure Migration

現在、本文・出版構造・共通部品・デザイン・theme・公開成果物を分離する移行を開始しています。
...
scripts/
  build / extract / validation

site/
  公開配信物
```

`publishing/` はparallel scaffoldです。現時点では公開 `site/` を変更せず、`_structured_build_preview/` にのみ生成します。

設計と停止条件は `ARCHITECTURE.md` および `publishing/README.md` を確認してください。
...
/
  ...
  scripts/
    ...
    extract_site_article_to_content.py
  site/
    index.html
    article/
    essay/
...
| `ARCHITECTURE.md` | Active | content / publishing / siteの責務、移行phase、gate、停止条件。 |
...
`refactor/publishing-structure-v0.1` branchでは、structured previewの検証が完了するまで `site/` を変更しません。

## README History

- 2026-07-22 22:50 JST: `content/`、`publishing/`、`scripts/`、`site/` の責務分離とparallel structured previewをRepository入口へ追加。`ARCHITECTURE.md` をActive文書化。
